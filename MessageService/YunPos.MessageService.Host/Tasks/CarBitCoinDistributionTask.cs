...
namespace YunPos.MessageService.Host.Tasks
{
    ...
    /// 车比特分配服务
    /// </summary>
    public class CarBitCoinDistributionTask : ITask
    {
        #region fields
        const string ScheduleJobIdentity = "CarBitCoinDistributionTask";
        string _apiDomain = string.Empty;
        log4net.ILog _logger;
        #endregion

        #region ctor.
        public CarBitCoinDistributionTask()
        {
        ...
        void RunSyncWeChatFans()
        {
            try
            {
                ...
                var requestUri = string.Format("{0}/api/BackgroundTask/DistributionCarBitCoin", _apiDomain);
        ...
        public void Start(string args)
        {
            try
            {
                ...
                {
                    _logger.Error("未配置 YunService，启动 车比特分配服务 失败。");
                    return;
                }
                ...
                {
                    _logger.Info("参数配置错误，正确的格式为 HH:mm，启动 车比特分配服务 失败。");
                    return;
                }
                int hour = 0, minute = 0;
