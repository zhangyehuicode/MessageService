...
namespace YunPos.MessageService.Host
{
    ...
    /// 卡券到期提醒服务
    /// </summary>
    public class CouponReminderTask : ITask
    {
        #region fields
        const string ScheduleJobIdentity = "CouponReminderTask";
        string _apiDomain = string.Empty;//api地址
        log4net.ILog _logger;
        #endregion

        #region ctor.
        public CouponReminderTask()
        {
            _apiDomain = AppContext.Settings.YunService;
            _logger = log4net.LogManager.GetLogger("CouponReminderTask");
        }
        #endregion
        ...
        void RunCouponReminder()
        {
            try
            {
                ...
                var requestUri = string.Format("{0}/api/BackgroundTask/CouponReminder", _apiDomain);
        ...
        public void Start(string args)
        {
            try
            {
                ...
                {
                    _logger.Error("未配置 YunService，启动 卡券到期提醒服务 失败。");
                    return;
                }
